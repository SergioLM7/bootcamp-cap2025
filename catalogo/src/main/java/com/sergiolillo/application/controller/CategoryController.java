...
public class CategoryController {
	...
	@Transactional
	public List<FilmDetailsDTO> getFilms(@PathVariable int id) throws NotFoundException {
		var Category = srv.getOne(id);
		if (Category.isEmpty())
			throw new NotFoundException();
		else {
			return (List<FilmDetailsDTO>) Category.get().getFilmCategories().stream()
					.map(item -> FilmDetailsDTO.from(item.getFilm())).collect(Collectors.toList());
		}
	}
	...
	@PostMapping
	@ApiResponse(responseCode = "201", description = "Category created", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Category.class), examples = {
